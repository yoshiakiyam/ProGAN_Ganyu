...
'''
Conv, Transpose, and linear layers with equal weight initialization
...
class ConvBlock(nn.Module):
    '''
    2 layers of:
    Equalconv,  pixelnorm, leaky relu
...
class Generator(nn.Module):
    ...
    def output(self, feat1, feat2, module1, module2, alpha):
        if 0 <= alpha < 1:
            ...
            #combine generated images from the previous layer plut the current layer to form one image
    ...
    def forward(self, input,  alpha=-1,step=0):
        # limit to max number of steps allowed
        if step > self.max_step:
            step = self.max_step

        #reshape noise to batch_size, input_dim, 1,1
        out_4 = self.input_layer(input.view(-1, self.input_dim, 1, 1))
        out_4 = self.progression_4(out_4)
        
        #feature doubled, then through conv layer
